...
public class PrinterHeadsHeadIdExtrudersExtruderIdHotendOffsetGetResponseParser {

    public static Offset parse(HttpResponse response) {
        if (response != null) {
            ...
            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK && entity.getContentType().getValue().equals(Settings.Connection.CONTENT_TYPE_JSON)) {
                return new Gson().fromJson(body, Offset.class);
            }
        }
        return null;
    }
