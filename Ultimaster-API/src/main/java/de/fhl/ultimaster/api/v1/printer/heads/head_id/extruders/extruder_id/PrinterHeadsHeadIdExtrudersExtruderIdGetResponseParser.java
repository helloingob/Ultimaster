...
public class PrinterHeadsHeadIdExtrudersExtruderIdGetResponseParser {

    public static Extruder parse(HttpResponse response) {
        if (response != null) {
            ...
            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK && entity.getContentType().getValue().equals(Settings.Connection.CONTENT_TYPE_JSON)) {
                return new Gson().fromJson(body, Extruder.class);
            }
        }
        return null;
    }
