...
public class HistoryPrintJobsUuidGetResponseParser {

    public static PrintJobHistoric parse(HttpResponse response) {
        if (response != null) {
            ...
            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK && entity.getContentType().getValue().equals(Settings.Connection.CONTENT_TYPE_JSON)) {
                return new Gson().fromJson(body, PrintJobHistoric.class);
            }
        }
        return null;
    }
