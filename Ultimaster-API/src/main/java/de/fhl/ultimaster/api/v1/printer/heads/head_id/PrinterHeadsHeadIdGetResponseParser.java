...
public class PrinterHeadsHeadIdGetResponseParser {

    public static Head parse(HttpResponse response) {
        if (response != null) {
            ...
            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK && entity.getContentType().getValue().equals(Settings.Connection.CONTENT_TYPE_JSON)) {
                return new Gson().fromJson(body, Head.class);
            } else {
                if (response.getStatusLine().getStatusCode() == HttpStatus.SC_NOT_FOUND) {
                    Logger.error("Head was not found. Note that this means that all deeper (eg: getting position, extruders, etc.) calls will also return a 404");
                }
            }
        }
        return null;
