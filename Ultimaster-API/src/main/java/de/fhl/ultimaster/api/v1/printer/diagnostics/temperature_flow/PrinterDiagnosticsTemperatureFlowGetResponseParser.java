...
public class PrinterDiagnosticsTemperatureFlowGetResponseParser {
    ...
    public static String parse(HttpResponse response) {
        if (response != null) {
            ...
            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK && entity.getContentType().getValue().equals(Settings.Connection.CONTENT_TYPE_CSV)) {
                return body;
            }
        }
        return null;
